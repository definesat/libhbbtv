...
NPObject *          APPPRIVDATA_Allocate(NPP npp, NPClass *theClass)
{
    ...
    newappprivdata->currentchannel = sBrowserFuncs->createobject(npp, fillCURRENTCHANNELpclass());
    debug("\t%s : Create keyset property at \x1b[%i;3%im%p\n\x1b[0m",__FUNCTION__, 1, 1, newappprivdata->keyset );
...
void        APPPRIVDATA_Deallocate(NPObject* obj)
{
    ...
    sBrowserFuncs->releaseobject(appprivdataobj->currentchannel);
...
bool        APPPRIVDATA_HasProperty(NPObject* obj, NPIdentifier name)
{
    ...
    if (!result)
        debug("APPPRIVDATA_HasProperty:%s\n", utf8propertyname);
    return result;
}
...
bool        APPPRIVDATA_GetProperty(NPObject* obj, NPIdentifier name, NPVariant* result)
{
	...
	else if (!strcmp(sBrowserFuncs->utf8fromidentifier(name), v_APPPRIVDATAPropertyNames[kAPPPRIVDATA_ID_PROPERTY_CURRENTCHANNEL]))
    {
		sBrowserFuncs->retainobject(AppPrivDataObj->currentchannel);
		OBJECT_TO_NPVARIANT(AppPrivDataObj->currentchannel, *result);
